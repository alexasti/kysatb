...
async def run():
    ...
    async with async_playwright() as p:
        ...
        while True:
            # Запрашиваем артикул у пользователя в начале выполнения скрипта
            TARGET_ARTICLE = input("\nВведите артикул товара для поиска (или 'exit' для выхода): ").strip()

            if TARGET_ARTICLE.lower() == "exit":
                print("Завершаем выполнение.")
                break  # Выход из цикла, завершение программы

            print(f"Ищем товар с артикулом {TARGET_ARTICLE}...")
