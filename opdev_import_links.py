...
# Предустановленные артикулы
default_articles = ["31904", "10487", "31220", "31500", "32216", "31879", "32304", "31499", "31878", "31923"]
...
async def run():
    ...
    excel_file = os.path.join(os.path.dirname(__file__), "Links.xlsx")
    ...
    async with async_playwright() as p:
        ...
        # Получаем список артикулов из данных скрипта или от пользователя
        arcticles_input = input("\nВведите артикулы для поиска через | (например 21611|26319|21606) или нажмите Enter для использования предустановленных: ").strip()

        # Если артикулы не введены, используем предустановленные
        if not arcticles_input:
            arcticles_input = "|".join(default_articles)  # Используем предустановленный список

        # Разбиваем строку на список артикулов
        articles_list = arcticles_input.split("|")

        for TARGET_ARTICLE in articles_list:
            TARGET_ARTICLE = TARGET_ARTICLE.strip()  # Убираем лишние пробелы

            print(f"\nИщем товар с артикулом {TARGET_ARTICLE}...")
            ...
            # Ищем предложенный товар с артикулом
            ...
            if not found:
                ...
                continue  # Продолжаем цикл, если товар не найден
            ...
            # Этап 6: Получаем основное изображение товара
            main_image = await page.query_selector('.img-fluid.product-images-main')  # Ищем изображение с классом "img-fluid product-images-main"
            main_image_url = await main_image.get_attribute('src') if main_image else "Изображение не найдено"
            main_image_url = "https://opdev.ru" + main_image_url if main_image_url and main_image_url.startswith('/') else main_image_url  # Добавляем базовый URL
            print(f"Основное изображение товара: {main_image_url}")

            # Ищем все кнопки с картинками
            buttons = await page.query_selector_all('.product-images-thumbs button')

            image_urls = [main_image_url]  # Добавляем основное изображение в список
            if not buttons:
            ...
            ws.append([product_name, article, ", ".join(image_urls)])
            ...
            wb.save(os.path.join(os.path.dirname(__file__), "Links.xlsx"))
            print("Результаты записаны в Excel файл: Links.xls")
