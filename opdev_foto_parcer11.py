...
import os
from openpyxl import Workbook  # Импортируем openpyxl для работы с Excel
...
async def run():
    ...
    ws.append(["Дата и время выполнения", "Название товара", "Артикул", "Характеристики", "Изображения"])
    ...
    async with async_playwright() as p:
        ...
        ws.append([datetime.now().strftime("%Y-%m-%d %H:%M:%S"), product_name, article, str(characteristics), ", ".join(image_urls)])
